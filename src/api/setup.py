...
3 - status (Статус, где 1 - в процессе, 2 - завершено, 3 - отложено)
4 - deadline (Крайний срок в формате YYYY-MM-DD)
"""
task_buttons = [
    ["Полить цветы", [], 1, 1, "2030-02-20"],  # Обычная задача с низким приоритетом
    ["Покормить кота", [], 2, 2, "2020-12-01"],  # Выполненная задача
    ["Закончить отчёт", ["Написать ввод", "Сделать расчёты"], 3, 1, "2025-03-25"],  # Важная задача с подзадачами
    ["Купить продукты", ["Молоко", "Хлеб", "Яйца"], 2, 1, "2025-03-18"],  # Список покупок
    ["Записаться к врачу", [], 3, 3, "2025-04-10"],  # Отложенная задача
    ["Позвонить маме", [], 1, 1, "2025-03-15"],  # Обычная задача
    ["Подготовиться к экзамену", ["Прочитать главы 1-3", "Решить тесты"], 3, 1, "2025-05-10"],  # Важная задача с подзадачами
    ["Оплатить счета", [], 2, 1, "2025-03-28"],  # Средний приоритет
    ["Прочитать книгу", [], 1, 3, "2025-06-01"],  # Отложенная задача
    ["Сделать зарядку", [], 2, 1, "2025-03-14"],  # На сегодня
]
